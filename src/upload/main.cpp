 ...
 *
 * NeHe - Crystal Space - Cube 3D - Quake and descendants - 
 *
...
void resize_window( int width, int height, int fov )
{
    ...
    */
    ...
    gluPerspective( (GLfloat)angle, ratio, 0.01f, 50000.0f );
...
//#define FRAME_TIME 8 // about 120 fps
#define FRAME_TIME 8 // about 200 fps
//#define PHYSICS_FRAME_TIME 10 // about 100 fps
        ...
        if ( delta_millis >= FRAME_TIME )
        {
            if ( engine.window_active )
            {
              if ( !engine.paused )
              {
                ...
                {
                    printf(" \n One seconds has gone by. %d frames were drawn. ", framecount ) ; 
                    printf(
        ...
        {
            //SDL_GL_SwapBuffers(); 
            // this is possible if the engine's window is not active - then 
            // delta_millis doesn't get 
            if ( !engine.window_active )
            {
                delta_millis = 0 ;
            }
